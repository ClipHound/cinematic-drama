...
| 队伍名称 | 给狗剪毛队 |
| 成员姓名 | 陈梓岩、马泽群、陈孟麟 |
| 技术栈 | 客户端：React 19、TypeScript、Vite 8、Tailwind CSS 4、React Router 7、Capacitor 8、Android WebView、Haptics/Share/Filesystem。<br>在线后端：Django 5、Django REST Framework、SQLite（开发）/ PostgreSQL（生产目标）、Celery/Redis（异步任务预留）。<br>AI 核心：Python Agent、Doubao-Seed-2.0-Lite / 火山方舟 OpenAI-compatible API、FunASR（可选 ASR）、Qdrant、Qwen3-Embedding / Ollama、Pydantic、json-repair。<br>数据与产物：Interaction Manifest JSON、rhythm_blueprint.json、Branch Narrative Package、用户互动事件、评论、收藏、搜索文档。 |
...
https://litmoon.cn/Demo-ByteDance/Video
...
| 后续优化 | 公开 GitHub、HTTPS 正式域名、真实生产数据库、任务队列、监控、更多剧集自动评测和 AB 测试。 |

## 项目代码和产物

### 代码链接

当前本地仓库未配置公开 GitHub 远端。提交前建议将以下占位替换为最终 GitHub 地址：
...
- Android 源码交付包：Android-App-Source-2026-06-11.zip

- Android 源码说明：ANDROID-SOURCE-README.md

- 前端与 Android 工程：cinematic-drama-app-frontend-source/

- 在线 Django 后端：django-backend/

- 离线短剧理解 Agent：drama-understanding-agent/

- 系统设计文档：sdd/SDD-00 至 sdd/SDD-09

- HTTPS 部署说明：HTTPS-DEPLOYMENT.md

### 主要 API

GET /api/dramas、GET /api/dramas/<slug>、GET /api/dramas/<slug>/episodes、GET /api/dramas/<slug>/episodes/<number>、GET /api/dramas/<slug>/episodes/<number>/interactions、GET /api/videos/<slug>/<number>、POST /api/interactions、GET /api/users/me/profile、GET/PUT/DELETE /api/users/me/favorites、GET/POST /api/comments、GET /api/search?q=、POST /api/ai/search、POST /api/ai/chat、POST /api/admin/dramas/upload、GET /api/admin/pipeline/jobs。
...
- 不足：公开 GitHub 链接仍需在提交前补齐；生产环境 HTTPS、数据库、任务队列和监控还需进一步完善；图像生成、更多剧集自动评测、互动热力统计、AB 测试和个性化推荐仍有提升空间。
